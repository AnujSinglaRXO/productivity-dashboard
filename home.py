...
with open("web-app/style.css") as f:
...
def make_monthly_issues_plot(df: pd.DataFrame) -> plt.figure:
    # try:
        ...
        plot_data = plot_data.groupby(["Month", "Issue Type"]).size().reset_index(name="Count")
        plot_data = plot_data.pivot(index="Month", columns="Issue Type", values="Count").fillna(0)
        ...
        if "Story" in plot_data.columns:
            fig.add_bar(x=plot_data.index, y=plot_data["Story"], name="Story", marker_color="limegreen", width=bar_width, offset=-bar_width)
        if "Task" in plot_data.columns:
            fig.add_bar(x=plot_data.index, y=plot_data["Task"], name="Task", marker_color="deepskyblue", width=bar_width, offset=-bar_width, base=plot_data["Story"] if "Story" in plot_data.columns else None)
        if "Defect (Standard)" in plot_data.columns:
            fig.add_bar(y=plot_data["Defect (Standard)"], x=plot_data.index, name="Defect (Standard)", marker_color="darkgrey", width=bar_width, offset=0)
        if "Bug" in plot_data.columns:
            fig.add_bar(x=plot_data.index, y=plot_data["Bug"], name="Bug", marker_color="red", width=bar_width, offset=0, base=plot_data["Defect (Standard)"] if "Defect (Standard)" in plot_data.columns else None)
        ...
        fig.update_layout(
            ...
            xaxis_ticktext=[f"{calendar.month_abbr[int(x.split('-')[1])]} {x.split('-')[0][-2:]}" for x in plot_data.index],
